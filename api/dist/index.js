...
__name(adminClient, "adminClient");
...
profiles.get("/me", requireAuth, async (c) => {
  ...
  const { data, error } = await supabase.from("profiles").select("*").eq("id", c.get("userId")).single();
...
profiles.patch("/me", requireAuth, async (c) => {
  ...
  const allowed = ["username", "display_name", "avatar_url", "bio", "is_private"];
  const updates = {};
  ...
  const supabase = userClient(c.env, c.get("userToken"));
  const { data, error } = await supabase.from("profiles").update(updates).eq("id", c.get("userId")).select().single();
...
profiles.get("/:id", async (c) => {
  ...
  const supabase = userClient(c.env, token);
  const { data, error } = await supabase.from("profiles").select("id, username, display_name, avatar_url, bio, is_private, created_at").eq("id", id).single();
